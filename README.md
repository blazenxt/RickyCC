...
- **🎨 Custom emoji icons** — restyle the bot's icons across every message body and the delivery caption: 67 named slots (`card`, `party`, `trophy`, `gift`, `validity`, …). Two ways, set from the panel:
  ...
  - **⚡ Premium Set, auto-loaded on boot** — a curated **67-icon premium look** (🎉🥇💎👑📌🔄…) is applied automatically when the bot starts on a fresh deploy (or one tap in the panel anytime) and replaces **every** Unicode icon across all message bodies — including the captcha page (a segment-aware `premiumize()` pass sweeps up even hardcoded literals, with zero double-wrapping). A built-in **live probe** first verifies the bot may send public custom emoji, so it can never break message delivery; an existing hand-made mapping is never overwritten.
