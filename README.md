...
- **🎨 Custom emoji icons** — restyle the bot's icons across every message body and the delivery caption: 30+ named slots (`card`, `party`, `trophy`, `gift`, `validity`, …). Two ways, set from the panel:
  ...
  - **⚡ one-tap Premium Set** — a curated 25-icon premium look (🎉🥇💎👑…) loads with a single button; a built-in **live probe** first verifies the bot may send public custom emoji, so it can never break message delivery
