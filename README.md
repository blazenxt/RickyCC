...
- **🎨 Custom emoji icons** — restyle the bot's icons across every message body and the delivery caption: 69 named slots (`card`, `party`, `trophy`, `gift`, `validity`, …). Two ways, set from the panel:
  ...
  - **⚡ Premium Set, auto-loaded on boot** — a curated **69-icon premium look** (🎉🥇💎👑📌🔄…) is applied automatically when the bot starts on a fresh deploy (or one tap in the panel anytime) and replaces **every** Unicode icon across all message bodies — including the captcha page (a segment-aware `premiumize()` pass sweeps up even hardcoded literals, with zero double-wrapping). A built-in **live probe** first verifies the bot may send public custom emoji, so it can never break message delivery; an existing hand-made mapping is never overwritten.
...
- **Multi-channel force subscribe** — comma-separated channel list, one join prompt listing every missing channel, referral payload survives the "Try again" flow. Membership checks for **all channels run in parallel** (no artificial delays) and invite links are pre-warmed at boot + cached, so /start answers near-instantly even with many channels. **Admin-approval channels supported**: for private channels/groups with "join by request", a *pending join request counts as joined* — the bot listens to `chat_join_request` updates and unlocks the user the moment they tap "Request to Join" (approval is outside the user's control, and getChatMember alone can't see pending requests).
- **Private chats only** — the bot never works in groups: the moment it's added to any group/supergroup it says a one-line goodbye and leaves automatically (the configured log/backup group is the single exception).
