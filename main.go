...
func main() {
	...
	dispatcher.AddHandler(handlers.NewConversation(
		[]ext.Handler{
			...
			handlers.NewCallback(callbackquery.Prefix("admc.alertsadd"), adminAlertsAddStart),
		},
		map[string][]ext.Handler{
			...
			admStateAlertsAdd: {handlers.NewMessage(anyText, adminAlertsAddMessage), handlers.NewCallback(callbackquery.Prefix("admcback"), adminConversationBack)},
		},
		&handlers.ConversationOpts{
			Exits:        []ext.Handler{handlers.NewCommand("cancel", adminCancel)},
			StateStorage: conversation.NewInMemoryStorage(conversation.KeyStrategySenderAndChat),
...
func start(b *gotgbot.Bot, ctx *ext.Context) error {
	// Private chats only: /start in a group gets a polite redirect — but the
	// bot NEVER leaves any group/channel on its own (fsub admin rights and
	// the log/backup destination depend on it staying put).
